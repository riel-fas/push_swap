...
/*   Updated: 2025/02/16 03:32:18 by riel-fas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void final_adjustments(t_stack_node **a)
{
    ...
    printf("Moving min node (%d) to top of stack A\n", min_node->nbr); // Debug statement
...
void sort_stack_b(t_stack_node **b)
{
    if (!b || !*b || !(*b)->next) // Ensure stack B has exactly 2 nodes
        return;
